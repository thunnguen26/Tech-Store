  ...
  // IMPORT SERVICE VÀ CÁC TYPE
  import { 
    ProductService, 
    AdminProductDetails 
  } from "@/services/ProductService"

  // Interface cho biến thể trong form
  ...
  console.log("🔍 DỮ LIỆU FEATURES (GỐC):", product.features);
  console.log("🔍 KIỂU DỮ LIỆU FEATURES:", typeof product.features);
    // State cho ảnh
    ...
    // === XỬ LÝ DỮ LIỆU SPECS (MỚI - Dựa trên log của bạn) ===
  let specs = {
    processor: '',
    ram: '',
    storage: '',
  ...
  // Kiểm tra xem features có phải là Mảng không (Đây là trường hợp của bạn)
  if (Array.isArray(product.features)) {
    product.features.forEach((item: any) => {
      // Chuyển về chuỗi để chắc chắn
      const strItem = String(item);
      
      // Tách chuỗi dựa trên dấu hai chấm đầu tiên
      // Ví dụ: "Bộ xử lý: Chip M4" -> ["Bộ xử lý", " Chip M4"]
      const separatorIndex = strItem.indexOf(':');
      
      if (separatorIndex !== -1) {
        const label = strItem.substring(0, separatorIndex).trim().toLowerCase();
        const value = strItem.substring(separatorIndex + 1).trim();

        // Ánh xạ từ Label tiếng Việt sang tên biến
        if (label.includes('bộ xử lý') || label.includes('cpu') || label.includes('chip')) {
          specs.processor = value;
        } else if (label.includes('ram')) {
          specs.ram = value;
        } else if (label.includes('bộ nhớ') || label.includes('dung lượng') || label.includes('rom')) {
          specs.storage = value;
        } else if (label.includes('màn hình') || label.includes('hiển thị')) {
          specs.screen = value;
        }
      }
    });
  } 
  // Trường hợp dự phòng: Nếu nó là JSON String
  else if (typeof product.features === 'string') {
    try {
      const parsed = JSON.parse(product.features);
      specs = {
        processor: parsed.processor || parsed.cpu || '',
        ram: parsed.ram || parsed.memory || '',
        storage: parsed.storage || parsed.rom || '',
        screen: parsed.screen || parsed.display || ''
      };
    } catch (e) {}
  }
  // Trường hợp dự phòng: Nếu nó là Object thuần
  else if (typeof product.features === 'object' && product.features !== null) {
    const parsed: any = product.features;
    specs = {
      processor: parsed.processor || parsed.cpu || '',
      ram: parsed.ram || parsed.memory || '',
      storage: parsed.storage || parsed.rom || '',
  ...
  // Hợp nhất dữ liệu: Ưu tiên lấy ở bảng products trước, nếu rỗng thì lấy từ specs vừa parse
  const finalSpecs = {
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                ...
                <Input 
                  id="processor" 
                  name="processor" 
                  defaultValue={finalSpecs.processor} // <-- Dùng finalSpecs
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <select
                    ...
                    className="mt-1.5 w-full h-11 rounded-lg border border-gray-800 bg-black px-3 text-white"
                    required
                  >
